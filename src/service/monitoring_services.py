import asyncio
from datetime import datetime, timedelta
from typing import List, Dict, Optional, Tuple, Any
from src.config.variable_config import MONITORING_CONFIG
from src.config.logger_config import logger
from src.service.funding_rate_service import (
    FundingRateService,
    get_funding_rate_service,
)
from src.service.btc_dominance_service import (
    BTCDominanceService,
    get_btc_dominance_service,
)
from src.service.etf_candlestick_service import (
    ETFCandlestickService,
    get_etf_candlestick_service,
)
from src.dto.funding_rate_dto import RealtimeFundingRateRequest
from src.dto.btc_dominance_dto import BTCDominanceRequest
from src.dto.etf_candlestick_dto import ETFCandlestickRequest


class FundingRateMonitoringService:
    """Service để check funding rate theo các chu kỳ 8h, 4h, 1h"""

    def __init__(self, funding_service: Optional[FundingRateService] = None):
        self.funding_service = funding_service or get_funding_rate_service()
        self.expected_symbols = MONITORING_CONFIG.get("expected_symbols", [])
        self.tolerance_minutes = MONITORING_CONFIG.get("tolerance_minutes", 30)

    def _get_funding_cycles(self) -> Dict[str, List[str]]:
        """Get funding rate cycles"""
        return {
            "8h": ["00:00:00", "08:00:00", "16:00:00"],
            "4h": [
                "00:00:00",
                "04:00:00",
                "08:00:00",
                "12:00:00",
                "16:00:00",
                "20:00:00",
            ],
            "1h": [f"{hour:02d}:00:00" for hour in range(24)],
        }

    def _get_current_funding_schedule(
        self, cycle_type: str
    ) -> Tuple[str, str, bool, str]:
        now = datetime.now()
        funding_times = self._get_funding_cycles()[cycle_type]
        today = now.strftime("%Y-%m-%d")

        for time_str in funding_times:
            expected_datetime = datetime.strptime(
                f"{today} {time_str}", "%Y-%m-%d %H:%M:%S"
            )

            if (
                expected_datetime
                <= now
                <= expected_datetime + timedelta(minutes=self.tolerance_minutes)
            ):
                return today, time_str, True, "current"

        for time_str in funding_times:
            expected_datetime = datetime.strptime(
                f"{today} {time_str}", "%Y-%m-%d %H:%M:%S"
            )
            if now < expected_datetime:
                return today, time_str, False, "upcoming"

        tomorrow = (now + timedelta(days=1)).strftime("%Y-%m-%d")
        return tomorrow, funding_times[0], False, "next_day"

    async def _check_funding_rate_data(
        self, symbols: List[str], expected_date: str, expected_time: str
    ) -> Dict[str, Any]:
        result = {}

        try:
            symbols_str = ",".join(symbols)
            request = RealtimeFundingRateRequest(symbols=symbols_str)
            response = await self.funding_service.get_realtime_funding_rate_data(
                request
            )

            for symbol in symbols:
                result[symbol] = {"has_data": False, "latest_record": None}

            for item in response.data:
                symbol = item.symbol
                update_date = item.update_date
                update_time = item.update_time

                latest_record = {
                    "symbol": symbol,
                    "funding_rate": item.funding_rate,
                    "update_date": update_date,
                    "update_time": update_time,
                }

                if update_date == expected_date:
                    try:
                        update_datetime = datetime.strptime(
                            f"{update_date} {update_time}", "%Y-%m-%d %H:%M:%S"
                        )
                        expected_datetime = datetime.strptime(
                            f"{expected_date} {expected_time}", "%Y-%m-%d %H:%M:%S"
                        )

                        time_diff = abs(
                            (update_datetime - expected_datetime).total_seconds()
                        )
                        if time_diff <= self.tolerance_minutes * 60:
                            result[symbol] = {
                                "has_data": True,
                                "latest_record": latest_record,
                            }
                        else:
                            result[symbol] = {
                                "has_data": False,
                                "latest_record": {
                                    **latest_record,
                                    "time_diff_minutes": round(time_diff / 60, 2),
                                },
                            }
                    except ValueError:
                        logger.warning(
                            f"Cannot parse time for {symbol}: {update_date} {update_time}"
                        )
                        result[symbol] = {
                            "has_data": False,
                            "latest_record": latest_record,
                        }
                else:
                    result[symbol] = {"has_data": False, "latest_record": latest_record}

        except Exception as e:
            logger.error(f"Error checking funding rate data: {str(e)}")

        return result

    async def check_funding_rate(self) -> Dict[str, Any]:
        logger.info("Checking funding rate data for all cycles...")

        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        cycles = ["8h", "4h", "1h"]

        result = {
            "timestamp": current_time,
            "cycles": {},
            "overall_status": "OK",
            "overall_alert_message": "",
            "total_symbols": len(self.expected_symbols),
        }

        try:
            for cycle in cycles:
                expected_date, expected_time, should_check, schedule_status = (
                    self._get_current_funding_schedule(cycle)
                )

                cycle_result = {
                    "expected_funding_time": f"{expected_date} {expected_time}",
                    "is_funding_time": should_check,
                    "schedule_status": schedule_status,
                    "symbols_with_data": 0,
                    "symbols_missing_data": 0,
                    "missing_symbols": [],
                    "symbols_details": {},
                    "status": "OK",
                    "alert_message": "",
                }

                if should_check:
                    symbols_data = await self._check_funding_rate_data(
                        self.expected_symbols, expected_date, expected_time
                    )
                    cycle_result["symbols_details"] = symbols_data

                    missing_symbols = []
                    for symbol in self.expected_symbols:
                        symbol_info = symbols_data.get(symbol, {"has_data": False})
                        if symbol_info["has_data"]:
                            cycle_result["symbols_with_data"] += 1
                        else:
                            cycle_result["symbols_missing_data"] += 1
                            missing_symbols.append(symbol)

                    cycle_result["missing_symbols"] = missing_symbols

                    if missing_symbols:
                        cycle_result["status"] = "WARNING"
                        cycle_result["alert_message"] = (
                            f"FUNDING RATE {cycle.upper()} ALERT: {len(missing_symbols)} symbols missing data at {expected_date} {expected_time}: {', '.join(missing_symbols)}"
                        )
                        logger.warning(cycle_result["alert_message"])

                        if result["overall_status"] == "OK":
                            result["overall_status"] = "WARNING"
                    else:
                        cycle_result["alert_message"] = (
                            f"All {len(self.expected_symbols)} symbols have funding rate data for {cycle} cycle"
                        )
                        logger.info(
                            f"All funding rate symbols have complete data for {cycle} cycle"
                        )
                else:
                    cycle_result["status"] = "NO_CHECK_NEEDED"
                    cycle_result["alert_message"] = (
                        f"Not in {cycle} funding rate check window. Next funding time: {expected_date} {expected_time}"
                    )

                result["cycles"][cycle] = cycle_result

            warnings = []
            for cycle, cycle_data in result["cycles"].items():
                if cycle_data["status"] == "WARNING":
                    warnings.append(
                        f"{cycle}: {cycle_data['symbols_missing_data']} symbols missing"
                    )

            if warnings:
                result["overall_alert_message"] = (
                    f"FUNDING RATE ALERTS - {', '.join(warnings)}"
                )
            else:
                active_cycles = [
                    cycle
                    for cycle, data in result["cycles"].items()
                    if data["is_funding_time"]
                ]
                if active_cycles:
                    result["overall_alert_message"] = (
                        f"All symbols have complete data for active cycles: {', '.join(active_cycles)}"
                    )
                else:
                    result["overall_alert_message"] = (
                        "No active funding rate checks at this time"
                    )

        except Exception as e:
            logger.error(f"Error during funding rate check: {str(e)}")
            result["overall_status"] = "ERROR"
            result["overall_alert_message"] = (
                f"Error checking funding rate data: {str(e)}"
            )

        return result


class BTCDominanceMonitoringService:

    def __init__(self, btc_service: Optional[BTCDominanceService] = None):
        self.btc_service = btc_service or get_btc_dominance_service()
        # Tolerance: nếu data cũ hơn 2 ngày thì coi như chưa được update
        self.max_days_old = 2

    async def check_btc_dominance(self) -> Dict[str, Any]:
        logger.info("Checking BTC dominance data freshness...")

        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        current_date = datetime.now()

        result = {
            "timestamp": current_time,
            "has_fresh_data": False,
            "latest_date": None,
            "alert_message": "",
            "status": "OK",
        }

        try:
            request = BTCDominanceRequest(days=0)
            response = await self.btc_service.get_btc_dominance_data(request)

            if response.data and len(response.data) > 0:
                logger.info(f"Total latest records received: {len(response.data)}")

                # Get the most recent record (first one since sorted by latest)
                latest_record = response.data[0]

                if latest_record.get("datetime"):
                    try:
                        if isinstance(latest_record["datetime"], str):
                            record_date_str = latest_record["datetime"][
                                :10
                            ]  # Get YYYY-MM-DD part
                        else:
                            record_date_str = latest_record["datetime"].strftime(
                                "%Y-%m-%d"
                            )

                        record_date = datetime.strptime(record_date_str, "%Y-%m-%d")

                        # Calculate days difference
                        days_difference = (current_date - record_date).days

                        result["latest_date"] = record_date_str

                        logger.info(
                            f"Latest BTC dominance record from {record_date_str}, {days_difference} days old"
                        )

                        if days_difference <= self.max_days_old:
                            result["has_fresh_data"] = True
                            result["status"] = "OK"
                            result["alert_message"] = (
                                f"BTC dominance data is fresh ({days_difference} days old, within {self.max_days_old} days tolerance)"
                            )
                            logger.info(result["alert_message"])
                        else:
                            result["status"] = "WARNING"
                            result["alert_message"] = (
                                f"BTC DOMINANCE ALERT: Data is stale ({days_difference} days old, exceeds {self.max_days_old} days tolerance)"
                            )
                            logger.warning(result["alert_message"])

                    except Exception as e:
                        logger.warning(
                            f"Cannot parse datetime from BTC record: {str(e)}"
                        )
                        result["status"] = "ERROR"
                        result["alert_message"] = (
                            f"BTC DOMINANCE ERROR: Cannot parse datetime from latest record"
                        )
                else:
                    result["status"] = "ERROR"
                    result["alert_message"] = (
                        "BTC DOMINANCE ERROR: Latest record has no datetime field"
                    )
            else:
                result["status"] = "ERROR"
                result["alert_message"] = (
                    "BTC DOMINANCE ALERT: No data found in database"
                )
                logger.error(result["alert_message"])

        except Exception as e:
            logger.error(f"Error checking BTC dominance: {str(e)}")
            result["status"] = "ERROR"
            result["alert_message"] = f"Error checking BTC dominance data: {str(e)}"

        return result


class ETFCandlestickMonitoringService:
    """Service để check ETF candlestick có data được update gần đây không"""

    def __init__(self, etf_service: Optional[ETFCandlestickService] = None):
        self.etf_service = etf_service or get_etf_candlestick_service()
        self.expected_symbols = [
            "E1VFVN30",
            "FUEABVND",
            "FUEBFVND",
            "FUEDCMID",
            "FUEFCV50",
            "FUEIP100",
            "FUEKIV30",
            "FUEKIVFS",
            "FUEKIVND",
            "FUEMAV30",
            "FUEMAVND",
            "FUESSV30",
            "FUESSV50",
            "FUESSVFL",
            "FUETCC50",
            "FUEVFVND",
            "FUEVN100",
        ]
        # Tolerance: nếu data cũ hơn 2 ngày thì coi như chưa được update
        self.max_days_old = 2

    async def check_etf_candlestick(self) -> Dict[str, Any]:
        """Check ETF candlestick có data được update gần đây không"""
        logger.info("Checking ETF candlestick data freshness...")

        current_time = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        current_date = datetime.now()

        result = {
            "timestamp": current_time,
            "total_symbols": len(self.expected_symbols),
            "symbols_with_fresh_data": 0,
            "symbols_with_stale_data": 0,
            "stale_symbols": [],
            "symbols_details": {},
            "alert_message": "",
            "status": "OK",
        }

        try:
            symbols_details = {}
            stale_symbols = []

            # Check each symbol
            for symbol in self.expected_symbols:
                try:
                    # Get latest records (day=0) for this symbol
                    request = ETFCandlestickRequest(day=0, symbol=symbol)
                    response = await self.etf_service.get_etf_candlestick_data(request)

                    symbol_info = {"has_fresh_data": False, "latest_date": None}

                    if response.data and len(response.data) > 0:
                        latest_record = response.data[0]

                        # Check how old the latest record is
                        record_datetime = (
                            latest_record.datetime
                            if hasattr(latest_record, "datetime")
                            else None
                        )
                        if record_datetime:
                            record_date_str = record_datetime[
                                :10
                            ]  # Get YYYY-MM-DD part
                            record_date = datetime.strptime(record_date_str, "%Y-%m-%d")

                            # Calculate days difference
                            days_difference = (current_date - record_date).days

                            symbol_info["latest_date"] = record_date_str

                            if days_difference <= self.max_days_old:
                                symbol_info["has_fresh_data"] = True
                                result["symbols_with_fresh_data"] += 1
                            else:
                                result["symbols_with_stale_data"] += 1
                                stale_symbols.append(symbol)
                        else:
                            result["symbols_with_stale_data"] += 1
                            stale_symbols.append(symbol)
                    else:
                        result["symbols_with_stale_data"] += 1
                        stale_symbols.append(symbol)

                    symbols_details[symbol] = symbol_info

                except Exception as e:
                    logger.warning(f"Error checking ETF symbol {symbol}: {str(e)}")
                    result["symbols_with_stale_data"] += 1
                    stale_symbols.append(symbol)
                    symbols_details[symbol] = {"has_fresh_data": False, "error": str(e)}

            result["stale_symbols"] = stale_symbols
            result["symbols_details"] = symbols_details

            if stale_symbols:
                result["status"] = "WARNING"
                result["alert_message"] = (
                    f"ETF CANDLESTICK ALERT: {len(stale_symbols)} symbols have stale data (older than {self.max_days_old} days): {', '.join(stale_symbols[:5])}{'...' if len(stale_symbols) > 5 else ''}"
                )
                logger.warning(result["alert_message"])
            else:
                result["status"] = "OK"
                result["alert_message"] = (
                    f"All {len(self.expected_symbols)} ETF symbols have fresh data (within {self.max_days_old} days)"
                )
                logger.info(
                    f"All ETF symbols have fresh data (within {self.max_days_old} days)"
                )

        except Exception as e:
            logger.error(f"Error checking ETF candlestick: {str(e)}")
            result["status"] = "ERROR"
            result["alert_message"] = f"Error checking ETF candlestick data: {str(e)}"

        return result


# Global service instances
_funding_rate_monitor = None
_btc_dominance_monitor = None
_etf_candlestick_monitor = None


def get_funding_rate_monitoring_service() -> FundingRateMonitoringService:
    """Singleton for FundingRateMonitoringService"""
    global _funding_rate_monitor
    if _funding_rate_monitor is None:
        _funding_rate_monitor = FundingRateMonitoringService()
    return _funding_rate_monitor


def get_btc_dominance_monitoring_service() -> BTCDominanceMonitoringService:
    """Singleton for BTCDominanceMonitoringService"""
    global _btc_dominance_monitor
    if _btc_dominance_monitor is None:
        _btc_dominance_monitor = BTCDominanceMonitoringService()
    return _btc_dominance_monitor


def get_etf_candlestick_monitoring_service() -> ETFCandlestickMonitoringService:
    """Singleton for ETFCandlestickMonitoringService"""
    global _etf_candlestick_monitor
    if _etf_candlestick_monitor is None:
        _etf_candlestick_monitor = ETFCandlestickMonitoringService()
    return _etf_candlestick_monitor
